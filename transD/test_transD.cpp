...
float *entityVec, *relationVec, *entityTransVec, *relationTransVec;
...
void prepre_binary() {
    ...
    if (stat((initPath + "A" + note + ".bin").c_str(), &statbuf3) != -1) {  
        ...
        memcpy(entityTransVec, ATmp, entityTotal * dimension * sizeof(float));
        memcpy(relationTransVec, ATmp + entityTotal * dimension, relationTotal * dimensionR * sizeof(float));
...
float calc_sum(long e1, long e2, long rel) {
    ...
    float sum = 0;
    float ee1 = 0, ee2 = 0, tmp1, tmp2;
    for (long ii = 0; ii < dimension; ii++) {
        ee1+= entityTransVec[last1 + ii] * entityVec[last1 + ii];
        ee2+= entityTransVec[last2 + ii] * entityVec[last2 + ii];
    }
    for (long ii = 0; ii < dimensionR; ii++) {
        tmp1 = ee1 * relationTransVec[lastr + ii];
        tmp2 = ee2 * relationTransVec[lastr + ii];
        if (ii < dimension) {
            tmp1 += entityVec[last1 + ii];
            tmp2 += entityVec[last2 + ii];
        }
        if (l1_flag)
            sum += fabs(tmp1 + relationVec[lastr + ii] - tmp2);
        else
            sum += (tmp1 + relationVec[lastr + ii] - tmp2)*(tmp1 + relationVec[lastr + ii] - tmp2);
    }
    return sum;
}
