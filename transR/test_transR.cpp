...
void* prepareMode(void *con) {
    ...
    int lef = entityTotal / (Threads) * id;
    int rig = entityTotal / (Threads) * (id + 1);
    if (id == Threads - 1) rig = entityTotal;
    for (int i = lef; i < rig; i++) {
        if (i % 100 == 0) printf("%d\n", i);
        for (int j = 0; j < relationTotal; j++) {
            int last = i * dimensionR * relationTotal + j * dimensionR;
            for (int k = 0; k < dimensionR; k++)
